    ...
    >
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={cred.is_active ? t("credentials.inUse") : t("credentials.activate", { name: cred.name })}
        ...
        <div className="flex flex-shrink-0 items-center gap-1">
          <button
            ...
            onClick={handleTest}
            disabled={testing}
            aria-label={t("credentials.testConnection", { name: cred.name })}
          ...
          {!isVertex && (
            <button
              ...
              aria-label={t("credentials.editLabel", { name: cred.name })}
              className={`rounded p-1.5 text-gray-500 transition-colors hover:bg-gray-800 hover:text-gray-300 ${focusRing}`}
            >
          ...
          {!confirmDelete ? (
            <button
              ...
              disabled={deleting}
              aria-label={t("credentials.deleteLabel", { name: cred.name })}
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {deleting ? <Loader2 className="h-3 w-3 animate-spin" /> : t("credentials.confirm")}
              </button>
              <button
                type="button"
        ...
        >
          ...
          {testResult.success && testResult.available_models.length > 0 && (
            <div className="mt-1 opacity-70">
              {t("credentials.availableModels")}: {testResult.available_models.join(", ")}
            </div>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="mt-2.5 ml-5.5 space-y-2.5 rounded-lg border border-gray-800 bg-gray-950/60 p-3">
          <div>
            <label htmlFor={`${editPrefix}-name`} className="mb-1 block text-xs text-gray-500">{t("credentials.name")}</label>
            <input
              id={`${editPrefix}-name`}
              name="name"
              type="text"
          ...
          <div>
            <label htmlFor={`${editPrefix}-apikey`} className="mb-1 block text-xs text-gray-500">{t("credentials.apiKeyKeepExisting")}</label>
            <input
              ...
              placeholder={t("credentials.keepExistingPlaceholder")}
              className={inputClsPlaceholder}
            />
          </div>
          {providerId === "gemini-aistudio" && (
            <div>
              <label htmlFor={`${editPrefix}-baseurl`} className="mb-1 block text-xs text-gray-500">{t("credentials.baseUrlOptional")}</label>
              <input
                ...
                placeholder={t("credentials.defaultUrlPlaceholder")}
                className={inputClsPlaceholder}
              />
            </div>
          )}
          <div className="flex gap-2 pt-0.5">
            ...
            >
              <X className="h-3 w-3" /> {t("credentials.cancel")}
            </button>
          </div>
        </div>
      )}
...
function AddCredentialForm({ providerId, isVertex, onCreated, onCancel }: AddFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isVertex) {
        ...
        if (!file) {
          setError(t("credentials.selectCredentialFile"));
          setSaving(false);
          return;
        }
        ...
        if (!apiKey.trim()) {
          setError(t("credentials.enterApiKey"));
          setSaving(false);
          return;
        }
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-950/60 p-3 space-y-2.5">
      <div>
        <label htmlFor="cred-add-name" className="mb-1 block text-xs text-gray-500">{t("credentials.name")} <span className="text-rose-400">*</span></label>
        <input
          id="cred-add-name"
          name="name"
          type="text"
      ...
      {isVertex ? (
        <div>
          <label htmlFor="cred-add-file" className="mb-1 block text-xs text-gray-500">{t("credentials.credentialFile")} <span className="text-rose-400">*</span></label>
          <button
            id="cred-add-file"
            type="button"
          ...
          >
            ...
            {fileRef.current?.files?.[0]?.name ?? t("credentials.selectJsonFile")}
          </button>
        ...
        <>
          <div>
            <label htmlFor="cred-add-apikey" className="mb-1 block text-xs text-gray-500">{t("credentials.apiKeyLabel")} <span className="text-rose-400">*</span></label>
            <input
              id="cred-add-apikey"
              name="api_key"
              type="password"
          ...
          {providerId === "gemini-aistudio" && (
            <div>
              <label htmlFor="cred-add-baseurl" className="mb-1 block text-xs text-gray-500">{t("credentials.baseUrlOptional")}</label>
              <input
                id="cred-add-baseurl"
                name="base_url"
                type="url"
  ...
  return (
    <div>
      <div className="mb-2.5 flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-300">{t("credentials.title")}</h4>
        {!showAdd && (
          <button
            type="button"
          ...
          >
            <Plus className="h-3 w-3" /> {t("credentials.addKey")}
          </button>
        )}
      </div>
      ...
      {credentials.length === 0 && !showAdd && (
        <div className="rounded-lg border border-dashed border-gray-700 px-4 py-6 text-center">
          <p className="text-sm text-gray-500">{t("credentials.noKeys")}</p>
          <button
            type="button"
            onClick={() => setShowAdd(true)}
          ...
          >
            <Plus className="h-3 w-3" /> {t("credentials.addFirstKey")}
          </button>
        </div>
      )}
