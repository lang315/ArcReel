...
function StatusBadge({ status }: { status: string }) {
  ...
  const STATUS_BADGE_MAP: Record<string, { label: string; cls: string }> = {
    ready: { label: t("providers.statusBadge.ready"), cls: "bg-green-900/30 text-green-400 border border-green-800/50" },
    unconfigured: { label: t("providers.statusBadge.unconfigured"), cls: "bg-gray-800 text-gray-400 border border-gray-700" },
    error: { label: t("providers.statusBadge.error"), cls: "bg-red-900/30 text-red-400 border border-red-800/50" },
  };
    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder={field.is_set ? field.value_masked ?? "••••••••••" : (field.placeholder ?? t("providers.detail.inputSecret"))}
            ...
            <button
              ...
              aria-label={showSecret ? t("providers.detail.hideSecret") : t("providers.detail.showSecret")}
            >
          ...
          {field.is_set && !confirmingClear && (
            <button
              type="button"
              onClick={handleClear}
              title={t("providers.detail.clearKey")}
          ...
          {confirmingClear && (
            <div className="flex items-center gap-1.5">
              ...
              >
                {t("providers.detail.confirmClear")}
              </button>
              <button
                type="button"
        ...
        {field.is_set && !(field.key in draft) && (
          <p className="mt-1 text-xs text-gray-600">{t("providers.detail.keySet")}</p>
        )}
      </div>
    );
  }
        ...
        <div className="mb-5 flex flex-wrap gap-1.5">
          {detail.media_types.map((mt) => (
            <span key={mt} className="rounded-md bg-gray-800 px-2 py-0.5 text-xs text-gray-400">
              {mt === "video"
                ? t("providers.detail.mediaType.video")
                : mt === "image"
                ? t("providers.detail.mediaType.image")
                : mt === "text"
                ? t("providers.detail.mediaType.text")
                : mt}
            </span>
          ))}
        </div>
      ...
      {detail.fields.length > 0 && (
        <div className="mt-6">
          ...
          >
            ...
            />
            {t("providers.detail.advancedConfig")}
          </button>
          {showAdvanced && (
                ...
                <div className="pt-2">
                  ...
                  >
                    {saving ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin" />
                        {t("providers.detail.saving")}
                      </>
                    ) : (
                      t("providers.detail.save")
                    )}
                  </button>
                </div>
              )}
