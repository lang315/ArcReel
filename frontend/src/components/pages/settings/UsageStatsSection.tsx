...
export function UsageStatsSection() {
  ...
  const TIME_RANGES = [
    { label: t("usage.last7Days"), days: 7 },
    { label: t("usage.last30Days"), days: 30 },
    { label: t("usage.all"), days: 0 },
  ];
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-100">{t("usage.title")}</h3>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {providers.length > 0 && (
          <select
            ...
            aria-label={t("usage.filterByProvider")}
      ...
      {/* Stats */}
      {loading ? (
        <div className="text-sm text-gray-500">{t("usage.loading")}</div>
      ) : stats.length === 0 ? (
        <div className="text-sm text-gray-500">{t("usage.noData")}</div>
      ) : (
          ...
          {stats.map((s) => (
            <div key={`${s.provider}-${s.call_type}`} className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-6 text-xs tabular-nums text-gray-400">
                <span>{t("usage.calls", { count: s.total_calls })}</span>
                <span>{t("usage.success", { count: s.success_calls })}</span>
                <span>
                  {t("usage.successRate")}{" "}
                  {s.total_calls > 0
                ...
                {s.call_type === "text" ? (
                  s.total_calls > 0 && <span>{t("usage.textGenType")}</span>
                ) : s.total_duration_seconds !== undefined && (
                  <span>{t("usage.duration", { seconds: s.total_duration_seconds })}</span>
                )}
              </div>
            </div>
          ))}
