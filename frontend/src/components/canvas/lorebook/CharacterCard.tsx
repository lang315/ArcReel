...
alt={t("character.characterSheetAlt", { name })}
          >
...
alt={t("character.characterSheetAlt", { name })}
                ...
                fallback={
                  <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-gray-500">
                    ...
                    <span className="text-xs">{t("character.clickToGenerate")}</span>
                  </div>
                }
              />
            </AspectFrame>
        ...
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            ...
            {(referenceFile || hasSavedReference) && (
              ...
              >
{referenceFile ? t("character.cancelPending") : t("character.replace")}
              </button>
            )}
          </div>
...
alt={t("character.referenceAlt", { name })}
  ...
  src={displayedReferenceUrl}
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/70 to-transparent px-3 py-2">
                  <span className="flex items-center gap-1.5 text-xs text-gray-200">
                    ...
                    {referenceFile ? t("character.pendingReference") : t("character.savedReference")}
                  </span>
                  <button
                    type="button"
      ...
      <label className="text-xs font-medium text-gray-400">{t("character.descriptionLabel")}</label>
      <textarea
        ref={textareaRef}
        value={description}
...
placeholder={t("character.descriptionPlaceholder")}
      ...
      <label className="mt-3 block text-xs font-medium text-gray-400">{t("character.voiceStyleLabel")}</label>
      <input
        type="text"
        value={voiceStyle}
...
placeholder={t("character.voiceStylePlaceholder")}
      />
      ...
      {isDirty && (
        ...
        >
{saving ? t("character.saving") : t("common:save")}
        </button>
      )}
      ...
      <div className="mt-3">
        <GenerateButton
          ...
          loading={generating}
label={character.character_sheet ? t("character.regenerateDesign") : t("character.generateDesign")}
