...
export function OverviewCanvas({ projectName, projectData }: OverviewCanvasProps) {
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      useAppStore.getState().pushToast(t("overview.uploadSuccess", { name: file.name }), "success");
    },
    [projectName],
  );
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      await API.generateOverview(projectName);
      await refreshProject();
      useAppStore.getState().pushToast(t("overview.regenerateSuccess"), "success");
    } catch (err) {
      useAppStore
        .getState()
        .pushToast(t("overview.regenerateFailed", { message: (err as Error).message }), "error");
    } finally {
  ...
  const handleStyleImageChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        await refreshProject();
        useAppStore.getState().pushToast(t("overview.styleImageUpdated"), "success");
      } catch (err) {
        useAppStore
          .getState()
          .pushToast(t("overview.styleImageUploadFailed", { message: (err as Error).message }), "error");
      } finally {
  ...
  const handleDeleteStyleImage = useCallback(async () => {
    ...
    if (!confirm(t("overview.confirmDeleteStyleImage"))) return;

    setDeletingStyleImage(true);
    try {
      await API.deleteStyleImage(projectName);
      await refreshProject();
      useAppStore.getState().pushToast(t("overview.styleImageDeleted"), "success");
    } catch (err) {
      useAppStore
        .getState()
        .pushToast(t("overview.styleImageDeleteFailed", { message: (err as Error).message }), "error");
    } finally {
  ...
  const handleSaveStyleDescription = useCallback(async () => {
    ...
    try {
      ...
      await refreshProject();
      useAppStore.getState().pushToast(t("overview.styleDescriptionSaved"), "success");
    } catch (err) {
      useAppStore
        .getState()
        .pushToast(t("overview.styleDescriptionSaveFailed", { message: (err as Error).message }), "error");
    } finally {
    ...
    <section className="rounded-2xl border border-gray-800 bg-gray-900/90 p-4 sm:p-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
        <div className="space-y-3">
          {styleImageUrl ? (
            <PreviewableImageFrame src={styleImageUrl} alt={t("overview.uploadAriaLabel")}>
              <div className="overflow-hidden rounded-xl border border-gray-700 bg-gray-950/70">
                <img
                  src={styleImageUrl}
                  alt={t("overview.uploadAriaLabel")}
            ...
            >
              ...
              <span>{uploadingStyleImage ? t("overview.uploading") : t("overview.uploadStyleImage")}</span>
              <span className="text-xs text-gray-600">{t("overview.supportedFormats")}</span>
            </button>
          )}

          <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-3">
            <p className="text-xs font-medium text-gray-400">{t("overview.usageNote")}</p>
            <p className="mt-1 text-sm leading-6 text-gray-300">
              {styleImageUrl
                ? t("overview.hasStyleImageNote")
                : t("overview.noStyleImageNote")}
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              >
                ...
                {styleImageUrl ? t("overview.replaceStyleImage") : t("overview.uploadRefImage")}
              </button>
              {styleImageUrl && (
                <button
                  type="button"
                ...
                >
                  ...
                  {deletingStyleImage ? t("overview.deleting") : t("overview.deleteStyleImage")}
                </button>
              )}
            </div>
          </div>
          ...
          <input
            ...
            className="hidden"
aria-label={t("overview.uploadAriaLabel")}
          />
        </div>

        <div className="rounded-xl border border-gray-800 bg-gray-950/35 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <label htmlFor="style-description-textarea" className="text-xs font-medium text-gray-400">{t("overview.styleDescription")}</label>
            <span className="text-[11px] text-gray-600">
{t("overview.charCount", { count: styleDescriptionDraft.trim().length })}
            </span>
          </div>
          <p className="mt-1 text-xs leading-5 text-gray-500">
{t("overview.styleDescAutoFill")}
          </p>
...
placeholder={t("overview.styleDescPlaceholder")}
          />

          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs leading-5 text-gray-500">
              {styleImageUrl
                ? t("overview.withStyleImageHint")
                : t("overview.withoutStyleImageHint")}
            </p>
            {styleDescriptionDirty && (
              ...
              >
                {savingStyleDescription ? t("overview.savingStyleDesc") : t("overview.saveStyleDesc")}
              </button>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            {projectData.content_mode === "narration"
              ? t("overview.contentModeNarration")
              : t("overview.contentModeDrama")}{" "}
            · {projectData.style || t("overview.noStyleSet")}
          </p>
        </div>
            ...
            {overview && (
              <div className="space-y-3 rounded-xl border border-gray-800 bg-gray-900 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-gray-300">{t("overview.projectOverview")}</h3>
                  <button
                    type="button"
...
title={t("overview.regenerateTitle")}
                  >
                    <RefreshCw
                      className={`h-3 w-3 ${regenerating ? "animate-spin" : ""}`}
                    />
                    <span>{regenerating ? t("overview.regenerating") : t("overview.regenerate")}</span>
                  </button>
                </div>
                <p className="text-sm text-gray-400">{overview.synopsis}</p>
                <div className="flex gap-4 text-xs text-gray-500">
                  <span>{t("overview.genre")}: {overview.genre}</span>
                  <span>{t("overview.theme")}: {overview.theme}</span>
                </div>
              </div>
            )}
                ...
                {(["characters", "clues"] as const).map(
                  (key) => {
                    ...
                    const labels: Record<string, string> = {
                      characters: t("overview.characters"),
                      clues: t("overview.clues"),
                    };
                    return (
                      <div
                        key={key}
...
<p className="text-sm text-gray-500 animate-pulse">{t("overview.calculatingCost")}</p>
              </div>
            )}
            {costError && (
              <div className="rounded-xl border border-red-900/50 bg-red-950/30 p-4">
<p className="text-sm text-red-400">{t("overview.costFailed", { error: costError })}</p>
              </div>
            )}

            {projectTotals && (
              <div className="rounded-xl border border-gray-800 bg-gray-900 p-4 tabular-nums">
<p className="mb-3 text-sm font-semibold text-gray-300">{t("overview.projectTotalCost")}</p>
                <dl className="flex flex-wrap items-start justify-between gap-6">
                  <div className="min-w-0">
                    <dt className="mb-1 text-[11px] text-gray-600">{t("overview.estimate")}</dt>
                    <dd className="text-sm text-gray-400">
                      <span className="text-gray-500">{t("overview.storyboard")} </span>
                      <span className="text-gray-200">{formatCost(projectTotals.estimate.image)}</span>
                      <span className="ml-3 text-gray-500">{t("overview.video")} </span>
                      <span className="text-gray-200">{formatCost(projectTotals.estimate.video)}</span>
                      <span className="ml-3 text-gray-500">{t("overview.total")} </span>
                  ...
                  <div className="min-w-0">
                    <dt className="mb-1 text-[11px] text-gray-600">{t("overview.actual")}</dt>
                    <dd className="text-sm text-gray-400">
                      <span className="text-gray-500">{t("overview.storyboard")} </span>
                      ...
                      {projectTotals.actual.character_and_clue && (
                        <>
                          <span className="ml-3 text-gray-500">{t("overview.characterAndClue")} </span>
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-gray-300">{t("overview.episodes")}</h3>
              {(projectData.episodes?.length ?? 0) === 0 ? (
                <p className="text-sm text-gray-500">
                  {t("overview.noEpisodes")}
                </p>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-500">
                        {ep.scenes_count ?? "?"} {t("overview.storyboard").replace(t("overview.storyboard"), "")} · {ep.status ?? "draft"}
                      </span>
                      {epCost && (
                        <span className="ml-auto flex min-w-0 flex-shrink flex-wrap gap-4 text-xs text-gray-400">
                          <span>
                            <span className="text-gray-500">{t("overview.episodeCostEstimate")} </span>
                            <span className="text-gray-500">{t("overview.storyboard")} </span><span className="text-gray-300">{formatCost(epCost.totals.estimate.image)}</span>
                            <span className="ml-2 text-gray-500">{t("overview.video")} </span><span className="text-gray-300">{formatCost(epCost.totals.estimate.video)}</span>
                            <span className="ml-2 text-gray-500">{t("overview.total")} </span><span className="font-medium text-amber-400">{formatCost(totalBreakdown(epCost.totals.estimate))}</span>
                          </span>
                          <span className="text-gray-700">|</span>
                          <span>
                            <span className="text-gray-500">{t("overview.episodeCostActual")} </span>
                            <span className="text-gray-500">{t("overview.storyboard")} </span><span className="text-gray-300">{formatCost(epCost.totals.actual.image)}</span>
                            <span className="ml-2 text-gray-500">{t("overview.video")} </span><span className="text-gray-300">{formatCost(epCost.totals.actual.video)}</span>
                            <span className="ml-2 text-gray-500">{t("overview.total")} </span><span className="font-medium text-emerald-400">{formatCost(totalBreakdown(epCost.totals.actual))}</span>
                          </span>
                        </span>
                      )}
                    </div>
