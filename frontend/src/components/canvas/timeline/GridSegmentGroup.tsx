      ...
      <div className="mb-3 flex items-center justify-between rounded-lg border border-amber-800/30 bg-amber-950/20 px-4 py-2.5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs text-gray-500">
            {t("grid.sceneCount", { count: sceneCount })}
            {gridInfo && batchCount > 1 ? (
              <>
                {" \u2192 "}
                <span className="font-mono text-amber-500/70">{t("grid.gridBatchCount", { count: batchCount })}</span>
        ...
        <motion.button
          ...
          title={!canGenerate ? t("grid.insufficientScenes") : undefined}
        ...
        >
          ...
          <span>{generatingGrid ? t("grid.generatingGrid") : t("grid.generateGrid")}</span>
        </motion.button>
      </div>
