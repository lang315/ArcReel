...
export function PreprocessingView({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setEditing(false);
      pushToast(t("preprocessing.savedSuccess"), "success");
    } catch {
      pushToast(t("preprocessing.saveFailed"), "error");
    } finally {
      setSaving(false);
    }
  ...
  const statusLabel =
    contentMode === "narration" ? t("preprocessing.segmentsDone") : t("preprocessing.sceneDone");

  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center text-gray-500">
        {t("preprocessing.loadingContent")}
      </div>
    );
  }
          ...
          {editing ? (
            <>
              ...
              >
                ...
                {saving ? t("preprocessing.saving") : t("preprocessing.save")}
              </button>
              <button
                type="button"
                onClick={() => {
