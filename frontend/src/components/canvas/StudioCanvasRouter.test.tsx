...
describe("StudioCanvasRouter", () => {
  ...
  it("runs character/clue callbacks and reports API failures with toast", async () => {
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("character.generateQueued");
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("clue.updateFailed");
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("clue.generateFailed");
    });
  ...
  it("runs timeline callbacks and handles generation failures", async () => {
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("timeline.updatePromptFailed");
    });
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("timeline.storyboardFailed");
    });
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("timeline.videoFailed");
    });
  });
});
