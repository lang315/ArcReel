...
const CALL_TYPE_CONFIG: Record<CallType, { icon: typeof Image; color: string; labelKey: string }> = {
  video: { icon: Video, color: "text-purple-400", labelKey: "usage.video" },
  text: { icon: FileText, color: "text-green-400", labelKey: "usage.text" },
  image: { icon: Image, color: "text-blue-400", labelKey: "usage.image" },
};
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-gray-800 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-gray-200">{t("usage.title")}</h3>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="grid grid-cols-5 gap-2 border-b border-gray-800 px-4 py-3">
        <StatBlock
          ...
          accent
        />
        <StatBlock label={t("usage.image")} value={String(stats?.image_count ?? 0)} icon={<Image className="h-3 w-3 text-blue-400" />} />
        <StatBlock label={t("usage.video")} value={String(stats?.video_count ?? 0)} icon={<Video className="h-3 w-3 text-purple-400" />} />
        <StatBlock label={t("usage.text")} value={String(stats?.text_count ?? 0)} icon={<FileText className="h-3 w-3 text-green-400" />} />
        <StatBlock label={t("usage.failed")} value={String(stats?.failed_count ?? 0)} icon={<AlertCircle className="h-3 w-3 text-red-400" />} />
      </div>

      {/* Call records */}
      <div className="max-h-72 overflow-y-auto">
        {callsLoading ? (
          <div className="flex items-center justify-center py-8 text-xs text-gray-500">{t("usage.loading")}</div>
        ) : calls.length === 0 ? (
          <div className="flex items-center justify-center py-8 text-xs text-gray-500">{t("usage.noRecords")}</div>
        ) : (
                ...
                <li key={call.id} className="px-4 py-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="flex-1 truncate text-xs text-gray-200" title={call.output_path ?? undefined}>
                      {filename || t(cfg.labelKey as "usage.video")}
                    </span>
                  ...
                  <div className="mt-0.5 flex items-center gap-2 pl-5.5 text-[10px] text-gray-500">
                    ...
                    {call.call_type === "text" ? (
                      <>
                        {call.input_tokens != null && <span>{t("usage.inputTokens", { count: call.input_tokens.toLocaleString() })}</span>}
                        {call.output_tokens != null && <span>{t("usage.outputTokens", { count: call.output_tokens.toLocaleString() })}</span>}
                      </>
                    ) : (
                      <>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-gray-800 px-4 py-2">
          <span className="text-[10px] text-gray-500">{t("usage.recordCount", { count: total })}</span>
