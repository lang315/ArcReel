...
function SessionSelector({
  ...
  const displayTitle = isDraftSession ? t("newSession") : (currentSession?.title || formatTime(currentSession?.created_at, t("newSession")));
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className="max-w-24 truncate">{displayTitle || t("noSession")}</span>
        ...
        >
          <div className="max-h-60 overflow-y-auto py-1">
            {sessions.map((session) => {
              ...
              const title = session.title || formatTime(session.created_at, t("newSession"));
              return (
                <div
                  key={session.id}
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); if (confirm(t("deleteSessionConfirm"))) onDelete(session.id); }}
...
function formatTime(isoStr: string | undefined, fallback: string): string {
  ...
  const addImages = useCallback((files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_IMAGE_BYTES) {
        setAttachError(t("imageTooLarge", { name: file.name }));
        continue;
      }
      ...
      <div className="border-t border-gray-800 p-3">
            ...
            {attachedImages.map((img) => (
              <div key={img.id} className="relative">
                <button
                  ...
                  aria-label={t("imagePreviewClick")}
                >
                  <img
                    src={img.dataUrl}
        ...
        >
          ...
          <button
            ...
            title={attachedImages.length >= MAX_IMAGES ? t("attachImageMax", { max: MAX_IMAGES }) : t("attachImage")}
